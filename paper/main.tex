...
answer it two ways. First, a general, effect-size-indexed seed-count lookup table
(Table~\ref{tab:seed_guide}) that any author or reviewer can apply directly to their own
comparison, no rerun of our analysis required: given an observed Cohen's $d$, it returns the
minimum seed count for conventional 80\% or 90\% power. Second, the empirical audit behind it:
four algorithms (PPO, SAC, TD3, A2C) on four MuJoCo robot-control tasks with twenty seeds each,
comparing naive small-$N$ conclusions against a rigorous bootstrap analysis (following Agarwal et
al.) on the full data. For a genuine effect, a 3-seed study reaches the wrong conclusion up to
\input{tables/PILOT4_headline_worst_k3.tex}\% of the time; for the smallest effect size we observe,
this failure rate stays at 95\% even using nineteen of our twenty seeds, and at the full twenty
seeds a conventional Welch's $t$-test misses two such effects entirely ($p=0.060$ and $p=0.269$,
on two different environments) that the rigorous method correctly detects in both cases. We
quantify these failure rates directly by exact combinatorial enumeration of every $k$-seed
sub-study ($k=3,\ldots,19$) drawable from our data, and explain the mechanism with a statistical
power analysis: the hardest case would need an estimated 126 seeds for conventional 80\% power,
far beyond what the seed-count guide's own textbook effect-size bands would suggest. We release
our full pipeline, data, and code to support further audits.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A statistical power analysis that explains this empirical result mechanistically, computing
    ...
    the seed count that would be required for adequate (80\%) power, and that produces the guide
    above.
...
\textbf{Reproducibility beyond RL.} The concerns we quantify here are not unique to reinforcement
learning. Pineau et al.~\cite{pineau2021improving}, reporting from the NeurIPS 2019 reproducibility
program, find incomplete reporting of experimental variance across machine learning broadly, not
just RL, and Lipton and Steinhardt~\cite{lipton2019troubling} identify the failure to distinguish
genuine improvement from noise as one of several recurring pathologies across the field's
scholarship as a whole. An exact, empirical quantification of how often a given seed budget
reaches the wrong conclusion, together with a general seed-count guide derived from it, is a
concrete instance of the fix these broader critiques call for, applied here to the robot-control
setting where the practical cost of an underpowered comparison is highest.
...
\input{tables/PILOT4_iqm_summary.tex}
...
\input{tables/PILOT4_aggregate.tex}
...
sample size, the two methods disagree in \input{tables/PILOT4_pairwise_disagree_counts.tex} of the
...
\input{tables/PILOT4_pairwise.tex}
...
\input{tables/PILOT4_smalln.tex}
...
\input{tables/PILOT4_seed_guide.tex}
...
results, is available at: \url{https://github.com/BenGreenberg07/rl-reproducibility-audit}.
