...
frequently fails to detect the effect at all. For pairs with no real effect (the PPO-vs-TD3
comparison; dashed line), the naive test's false-positive rate tracks the nominal $\alpha=0.05$
threshold closely across all $k$, exactly as a well-calibrated test should.
...
\begin{figure}[t]
  ...
  effect (disagreement is a missed detection). Dashed markers: the one pair with no detectable
...
gives reasonable power, while at least one pair (PPO vs.\ TD3 on Reacher-v5) has a small-to-medium
effect size for which even our full $n=10$ sample is underpowered, explaining why our own rigorous
analysis correctly declines to call that comparison significant: the honest conclusion is not that
the two algorithms are equivalent, but that we lack the statistical power, even at $n=10$, to
resolve whichever modest difference (if any) exists between them.
...
Second, when an effect is modest, as with PPO versus TD3 in our data, no seed count in the range
typically used in the literature is likely to resolve it (Table~\ref{tab:power}). A ``no
significant difference'' finding under these conditions should be reported as inconclusive due to
insufficient power, not as evidence of equivalence.
