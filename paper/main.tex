...
robot tasks (Reacher-v5 and HalfCheetah-v5) with twenty independent seeds per algorithm-task pair,
...
following the methodology of Agarwal et al.) computed on the full data; even at twenty seeds, one
of twelve algorithm-pair comparisons shows a genuine disagreement, a naive $t$-test that narrowly
misses significance ($p=0.060$) on an effect the rigorous method correctly identifies as real. We
further quantify, by exact combinatorial enumeration of all possible $k$-seed subsets
($k=3,\ldots,19$) of our data, how often a study using only $k$ seeds would have reached a
different conclusion than the full-data answer, and complement this empirical result with a
statistical power analysis explaining the mechanism. We find that for algorithm pairs with a
genuine, statistically robust performance gap, a 3-seed study would have missed or misjudged the
difference in up to \input{tables/PILOT_headline_worst_k3.tex}\% of possible seed selections; for
the pair with the smallest effect size, this failure rate remains above 70\% even using nineteen
of our twenty seeds. We release our full pipeline, data, and analysis code to support further
audits of this kind on other benchmarks and algorithms.
\end{abstract}
